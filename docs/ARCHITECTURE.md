...
As tres MAs visuais sao reunidas em `VisualIndicators/FusionVisualMA.mq5`, embutido como recurso no executavel principal. As cores de rapida, lenta, Trend e Bollinger sao preferencias visuais persistidas no perfil e selecionadas por swatches em `CONFIG > SYSTEM`. A paleta combina cores claras e escuras, incluindo azul-marinho, gold, verde-escuro e vermelho-escuro; perfis sem essas chaves usam verde, vermelho, fucsia e azul. As bandas visuais usam `VisualIndicators/FusionVisualBands.mq5`; o RSI visual usa `VisualIndicators/FusionVisualRSI.mq5`. Para um build limpo, compile primeiro os tres indicadores visuais e depois `Fusion.mq5`; em execucao, basta distribuir o `Fusion.ex5`, que ja contem os recursos.
