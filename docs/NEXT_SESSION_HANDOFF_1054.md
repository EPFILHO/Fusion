...
Estado mais recente: `CONFIG > SYSTEM` possui um toggle persistente para indicadores visuais e swatches persistentes para as cores da MA rapida, lenta, Trend e Bollinger, organizados em grade 2x2. O componente usa handles separados e reconcilia mudancas em duas fases: primeiro limpa e confirma a ausencia de indicadores pertencentes ao Fusion; num ciclo posterior recria o conjunto. MA rapida, MA lenta, Trend, RSI e BB aparecem somente quando o timeframe do grafico coincide com o TF configurado; nao existe projecao visual MTF. A legenda informa periodo, TF e estado sem consultar buffers pelo timer. A limpeza faz um unico passe por ciclo, a verificacao de orfaos ocorre a cada cinco segundos e a sincronizacao visual fica depois das tarefas operacionais e da GUI. O RSI visual mostra os niveis ativos, tipados como inteiros, e reune niveis de estrategia/filtro quando compartilham a curva. Os recursos `VisualIndicators/FusionVisualMA.mq5`, `FusionVisualBands.mq5` e `FusionVisualRSI.mq5` devem ser compilados antes de `Fusion.mq5`; perfis antigos mantem os defaults verde, vermelho, fucsia e azul.
