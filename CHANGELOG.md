...
- O painel passou a sinalizar quando o perfil ativo nao tem mais arquivo em disco: o cabecalho mostra `nome (sem arquivo)` em amarelo e a aba `STATUS` explica que o EA segue com os valores do estado do grafico e que basta salvar para recriar o arquivo. Antes o cabecalho exibia um perfil que ja nao aparecia na lista, sem nenhuma indicacao. A verificacao roda no timer de 1 s, junto das demais checagens de perfil, e nao no caminho do tick.
- Carregar um perfil pelo painel passou a ser registrado em log com Magic e lote. Antes so os carregamentos que falhavam eram registrados, entao uma troca manual de perfil nao deixava rastro algum.
- O vinculo entre grafico e perfil deixou de se perder junto com o estado de runtime. O estado salvo por grafico carrega duas coisas de naturezas diferentes: o estado operacional (posicao, streak, contadores, drawdown), que pode ser descartado porque o EA ressincroniza no boot, e a identidade do perfil, que nao pode, porque adotar outro perfil muda lote e Magic Number. Enquanto compartilharam o mesmo criterio, qualquer rejeicao do estado fazia o EA cair no perfil de inicializacao em silencio. Agora o nome do perfil e publicado mesmo quando a validacao do runtime falha e prevalece sobre o perfil de inicializacao.
- Quando o perfil que o grafico usava nao pode ser carregado, o EA passa a **bloquear** em vez de assumir outro. O Fusion nao escolhe perfil sozinho quando a escolha muda risco.
- A resolucao de perfil no boot passou a ser registrada em log com Magic e lote ativos, saindo como aviso sempre que nao for uma restauracao direta. Antes, o EA so registrava perfil quando um carregamento falhava, e o unico aviso que cobria estado nao restaurado era condicionado a o perfil de inicializacao tambem ter falhado; com ele carregando normalmente, a troca era completamente silenciosa.
- Avisos de contexto deixaram de ser suprimidos com o AutoTrading desligado. Essa condicao escondia justamente o pior caso, ja que o MetaTrader desliga o AutoTrading ao trocar de conta.
- `INICIAR` voltou a ficar disponivel com posicao aberta. A permissao de rearmar o EA compartilhava a trava de edicao de configuracao, que existe para impedir alterar lote ou SL durante uma operacao; como autorizar novas entradas nao altera configuracao, `canStart` deixou de depender de `runtimeEditable`. Antes, apos reabrir o MetaTrader com posicao em curso, era preciso esperar a operacao fechar para conseguir rearmar. As demais guardas do botao seguem intactas: perfil em uso por outro grafico, configuracao invalida, alteracoes pendentes, bloqueio de contexto e permissao de negociacao. A retomada continua exigindo clique manual, por decisao registrada em `docs/DECISIONS.md`.
- A navegacao do painel passou a usar visibilidade diferencial. Antes, cada clique de aba ou subaba reaplicava visibilidade em todos os grupos de controle de todas as abas; agora o clique esconde apenas o bloco que sai e mostra apenas o que entra. A varredura completa continua em uso na criacao do painel e ao maximizar, onde e necessaria.
- A ordem de pintura ficou uniforme em todos os caminhos de navegacao: primeiro o estado logico, depois validacao/sincronizacao/estilos com o bloco de destino ainda oculto, e por ultimo a exibicao ja no estado final. Isso elimina a repintura visivel ao entrar em `STRATEGIES` e `FILTERS`, que reescreviam todos os controles depois de ja estarem na tela.
- `inp_ShowPanel` voltou a valer no Strategy Tester. O painel era forcado em modo visual independentemente do input; agora o input decide em qualquer contexto e o modo visual e apenas um pre-requisito adicional no tester.
- Indicadores deixaram de ser desenhados em backtest: `TesterHideIndicators(true)` e chamado assim que o tester e detectado, antes de qualquer handle, suprimindo o desenho automatico dos indicadores operacionais pelo terminal; e o `CChartIndicatorVisualizer` nao sincroniza no tester. Comportamento em grafico real permanece identico.
- `Core/EAApplication.mqh` (2330 linhas) foi dividido em doze fragmentos de corpo de classe (`EAApplicationPartials`, `InstanceGuard`, `Modules`, `Snapshot`, `ProtectionNotice`, `EntryBlock`, `ProfileBlock`, `Settings`, `Entry`, `PositionSync`, `ManagePosition` e `Commands`), seguindo a mesma organizacao ja usada em `UIPanel.mqh`. O arquivo principal mantem as declaracoes de modulos e estado e o ciclo de vida da plataforma; a ordem das linhas foi preservada e a concatenacao reproduz o arquivo anterior byte a byte.
...
- Primeira limpeza curta da Fase 1: o reset de flags/avisos transitorios do `EAApplication`, a checagem de janelas de News no snapshot e a atualizacao condicional do painel passaram a reaproveitar helpers internos, sem mudanca de fluxo operacional.
- `EAApplication` passou a reaproveitar um helper unico para recarregar settings no painel apos salvar/carregar perfil, mantendo intactos locks, `ApplySettings` e persistencia.
- `Fusion.mq5` passou a declarar `Copyright 2026, EP Filho` e link do GitHub do projeto, e o README esclarece que o reload de configuracao e controlado a frio, sem mudancas livres em plena operacao.
- `STATUS`, `RESULTS` e `CONFIG > PROTECT > GERAL` tiveram seus papeis separados: STATUS mostra contexto operacional, RESULTS concentra metricas diarias e PROTECT resume apenas o estado das protecoes.
- `STATUS` agora mostra ativo e timeframes do contexto operacional carregado, sem acompanhar indevidamente mudancas visuais do grafico; rotulos e estados remanescentes foram traduzidos para portugues ASCII.
- `CONFIG > SYSTEM` passou a exibir `Resolver Conflito`, `PRIORIDADE`/`CANCELAR` e rodape explicando que o maior numero de prioridade vence.
- Apos um fechamento completo, sinais acumulados de todas as estrategias sao descartados antes da proxima decisao normal, evitando entrada imediata de outra estrategia no mesmo candle; a reversao explicitamente armada permanece no fluxo proprio.
