   ...
   void              SyncGuidance(const SEASettings &settings,const bool editable)
     {
      ...
      m_noteHint.Text(settings.bbFilterSlopeDirectionEnabled
                      ? "Direção=inclinação média da linha central em candles fechados"
   ...
   bool              Create(CFusionPanel *parent,const long chartId,const int subwin,const int x1,const int y1,const int x2,const int y2)
     {
      ...
      if(!m_slopeLookback.Create(parent, chartId, subwin, prefix + "slope_lookback", "Candles", x1 + 206, y1 + 244, x1 + 280, y1 + 262, x1 + 292, y1 + 240, x1 + 392, y1 + 264, 3))
         return false;
      if(!m_minSlopePoints.Create(parent, chartId, subwin, prefix + "slope_min", "Incl. mín. (pts/candle - Zero=mais sensível)", x1, y1 + 284, x1 + 280, y1 + 302, x1 + 292, y1 + 280, x1 + 392, y1 + 304, 0))
         return false;
